...
const crmPipeline = {
  leads: [
    { id: 1, name: "TechCorp Solutions", email: "contact@techcorp.com", status: "New", value: 25000 },
    { id: 2, name: "Digital Marketing Pro", email: "hello@dmp.com", status: "Contacted", value: 15000 }
  ],
  prospects: [
    { id: 3, name: "StartupXYZ", email: "info@startupxyz.com", status: "Qualified", value: 35000 },
    { id: 4, name: "Innovation Labs", email: "contact@innovationlabs.com", status: "Proposal", value: 20000 }
  ],
  clients: [
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <div className="border-2 border-dashed border-neutral-300 rounded-lg p-8 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-neutral-900 mb-2">Upload Your Logo</h3>
                    <p className="text-neutral-600 mb-4">Upload an existing logo or create one with AI</p>
            ...
            <CardContent>
              <div className="space-y-4">
                {marketingCampaigns.map((campaign) => (
                  <div key={campaign.id} className="flex items-center justify-between p-4 border border-neutral-200 rounded-lg">
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="text-right">
                        <p className="text-sm text-neutral-600">Conversions: {campaign.conversions}</p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Leads */}
                <div>
                  <h4 className="font-medium text-neutral-900 mb-4">Leads ({crmPipeline.leads.length})</h4>
                  <div className="space-y-3">
                    {crmPipeline.leads.map((lead) => (
                      <div key={lead.id} className="p-3 border border-neutral-200 rounded-lg">
                        ...
                        <p className="text-sm font-semibold text-neutral-900">₹{lead.value.toLocaleString()}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Prospects */}
                <div>
                  <h4 className="font-medium text-neutral-900 mb-4">Prospects ({crmPipeline.prospects.length})</h4>
                  <div className="space-y-3">
                    {crmPipeline.prospects.map((prospect) => (
                      <div key={prospect.id} className="p-3 border border-neutral-200 rounded-lg">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-sm font-medium text-neutral-900">{prospect.name}</span>
                        </div>
                        <p className="text-sm text-neutral-600 mb-2">{prospect.email}</p>
                        <p className="text-sm font-semibold text-neutral-900">₹{prospect.value.toLocaleString()}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Clients */}
                <div>
                  <h4 className="font-medium text-neutral-900 mb-4">Clients ({crmPipeline.clients.length})</h4>
                  <div className="space-y-3">
                    {crmPipeline.clients.map((client) => (
                      <div key={client.id} className="p-3 border border-neutral-200 rounded-lg">
                        ...
                        <p className="text-sm font-semibold text-neutral-900">₹{client.value.toLocaleString()}</p>
                      </div>
                    ))}
                  </div>
                </div>
