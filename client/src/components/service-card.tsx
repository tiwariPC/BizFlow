...
import { User, Users, Handshake, Bus, Building, TrendingUp, LucideIcon } from 'lucide-react';
...
export function ServiceCard({ package: pkg, onSelect }: ServiceCardProps) {
  ...
  return (
    <Card
      className='relative hover:shadow-lg transition-shadow cursor-pointer group h-full'
      data-testid={`service-card-${pkg.id}`}
    >
      {pkg.popular && (
      ...
      <CardContent className='p-6 h-full flex flex-col'>
        <div className='mb-4'>
          ...
          <h3
            ...
            data-testid={`service-title-${pkg.id}`}
          >
            {pkg.name}
          </h3>
          <p
            className='text-neutral-600 text-sm mb-4'
            data-testid={`service-description-${pkg.id}`}
          >
            {pkg.description}
          </p>
        </div>
